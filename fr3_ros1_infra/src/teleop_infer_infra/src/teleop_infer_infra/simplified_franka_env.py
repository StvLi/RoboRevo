...
class SimplifiedFrankaEnv:
    ...
    def __init__(self, ref_frame = 'base', debug=True):
        """
    ...
    def retarget(self, action=None, buttons=None):
        ...
        try:
            ...
            if self.ref_frame == "base" or self.ref_frame == "space":
                # 【默认】相对于基座/空间的增量控制：如异构臂控臂
                # action为基座系下表示的末端坐标系增量
                base_action_position = action[:3]
                base_action_rotation = action[3:6]
            elif self.ref_frame == "ee" or self.ref_frame == "body":
                # 【可选】相对于末端/本体的增量控制：如UMI夹爪
                # action为末端系下表示的末端坐标系增量
                base_action_position = last_target_pose['orientation'].as_matrix() @ action[:3]
                base_action_rotation = action[3:6]
            else:
                base_action_position = np.array([0,0,0])
                base_action_rotation = np.array([0,0,0])
            
            # 计算新的平衡位置
            new_target_position = last_target_pose['position'] + base_action_position
            new_target_rotation = last_target_pose['orientation'] * R.from_rotvec(base_action_rotation)
            ...
            # elif self.ref_frame == "ee" or self.ref_frame == "body":
            #     # 使用动作的前3维更新位置
            #     new_position = last_target_pose['position'] + last_target_rotation.as_matrix() @ action[:3]
                
            #     # 创建动作对应的旋转
            #     action_rotation = R.from_rotvec(last_target_rotation.as_matrix() @ action[3:6])
    ...
    def publish_equilibrium_tf(self):
        """
        发布平衡pose的tf变换
        
        此函数在每次被调用时发布平衡pose（target_pose）的tf变换。
        tf变换从基坐标系（panda_link0）到平衡pose坐标系（equilibrium_pose）。
        """
        try:
            if self.target_pose is None:
                rospy.logwarn("目标位姿不可用，无法发布tf变换")
                return
            ...
            # 将scipy Rotation转换为四元数 [x, y, z, w]
            quaternion = orientation.as_quat()
            
            # 发布tf变换
            # 父坐标系: panda_link0 (基坐标系)
            # 子坐标系: equilibrium_pose (平衡pose坐标系)
            self.tf_broadcaster.sendTransform(
                translation=(position[0], position[1], position[2]),
                rotation=(quaternion[0], quaternion[1], quaternion[2], quaternion[3]),
                time=current_time,
                child="equilibrium_pose",
                parent="fr3_link0"
            )
            
            if self.debug:
                rospy.logdebug(f"已发布平衡pose的tf变换: 位置={position}, 四元数={quaternion}")
                
        except Exception as e:
            rospy.logerr(f"发布平衡pose的tf变换时出错: {e}")
